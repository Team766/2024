...
public class Drive extends Mechanism {
     ...
     */
    private final double ENCODER_CONVERSION_FACTOR =
    ...
    public Drive() {
        ...
        motorList = new MotorController[] {m_DriveFR, m_DriveFL, m_DriveBL, m_DriveBR};
    ...
    public void controlModuleSteer(MotorController steerMotor, Vector2D vector, double offset) {
        ...
        final double angleDegrees =
                vectorTheta
                        + 360
                                * (Math.round(
                                        (steerMotor.getSensorPosition() / ENCODER_CONVERSION_FACTOR
                                                        - offset
                                                        - vectorTheta)
                                                / 360))
                        + offset;
        ...
        // Needs to multiply by encoderconversionfactor to translate into a unit the motor
        // understands
     ...
     * @param offset the offset for this module
     */
    public void controlModuleSteerAndPower(
    ...
    public void setEncoderOffset() {
        offsetFR =
                (m_SteerFR.getSensorPosition() / ENCODER_CONVERSION_FACTOR) % 360
                        - e_FrontRight.getAbsolutePosition();
        offsetFL =
                (m_SteerFL.getSensorPosition() / ENCODER_CONVERSION_FACTOR) % 360
                        - e_FrontLeft.getAbsolutePosition();
        offsetBR =
                (m_SteerBR.getSensorPosition() / ENCODER_CONVERSION_FACTOR) % 360
                        - e_BackRight.getAbsolutePosition();
        offsetBL =
                (m_SteerBL.getSensorPosition() / ENCODER_CONVERSION_FACTOR) % 360
                        - e_BackLeft.getAbsolutePosition();
    }
